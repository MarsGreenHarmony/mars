...
function writeProc() {
    ...
    if (!fileInput1.files[0]) {
        alert('상품 설명 이미지를 선택하세요.');
        fileInput1.focus();
        return;
    }

    var frmMain = document.getElementById("frmMain");
    frmMain.action = "/seller/sale/writeProc";
    frmMain.submit();
}
</script>
		...
		<div class="main-panel">
			<div class="content-wrapper">
				<div class="page-header">
					<h3 class="page-title">판매등록</h3>
				</div>
				<div class="row">
					<div class="col-12 grid-margin stretch-card">
						<div class="card">
							<div class="card-body">
								<h4 class="card-title">판매 등록하기</h4>

								<!-- form 태그 시작 -->
								<form id="frmMain" class="forms-sample">
									<div class="form-group">
										<label for="sel_nm">판매명</label> <input type="text"
											class="form-control" id="sel_nm" placeholder="상품명"
											name="sel_nm">
									</div>
									<!-- 임시주석@@@@@@ -->
									<!-- <div class="form-group">
									...
									<div class="form-group">
										<label for="price">판매가격</label> <input
											...
											placeholder="ex) 10000원이라면 10000 숫자만 입력" name="price">
									</div>
									...
									<div class="form-group">
										<label>등록상품</label>
										<c:choose>
											<c:when test="${empty listProduct}">판매 가능한 상품이 없습니다.</c:when>
											<c:otherwise>
												<select name="seq_prd" class="form-control">
									 ...
									 <div class="form-group" id="img">
										<label>상품 설명 이미지 등록(최대10개)</label>
										<div class="filebox">
											<div id="imageContainer1"></div>
											<input class="upload-name" placeholder="첨부파일" name="sel_img">
											<label for="fileInput1">파일찾기</label> <input type="file"
												id="fileInput1" accept="image/*" name="file1"
												onchange="uploadImage(1);">
										</div>
									</div>
									<button class="btn btn-inverse-primary btn-fw" type="button" onclick="addImageInput()">이미지 추가</button>

									<script>
									
									var imageCount = 1; // 최대 이미지 개수

									function addImageInput() {
									    if (imageCount < 10) { // 이미지 개수 제한 없애려면 최대 개수를 적절히 늘리세요
									        imageCount++; // 이미지 개수 증가

									        var newInput = document.createElement('div');
									        newInput.classList.add('filebox');
									        newInput.innerHTML = '<div id="imageContainer' + imageCount + '"></div>' +
									                             '<input class="upload-name" placeholder="첨부파일" name="sel_img">' +
									                             '<label for="fileInput' + imageCount + '">파일찾기</label>' +
									                             '<input type="file" id="fileInput' + imageCount + '" accept="image/*" name="file' + imageCount + '" onchange="uploadImage(' + imageCount + ');">';

									        document.getElementById('img').appendChild(newInput);

									        if (imageCount >= 10) {
									            document.querySelector('button').disabled = true; // 이미지 개수가 최대일 때 버튼 비활성화
									        }
									    }
									}
								...
								function uploadImage(input) {
								    ...
								    var imageContainerId = input.id.replace('fileInput', 'imagePreview');
								    
								    if (file) {
								        var reader = new FileReader();
								        reader.onload = function(e) {
								            // 선택된 이미지를 표시
								            var imagePreview = document.getElementById(imageContainerId);
								            imagePreview.innerHTML = `<img src="${e.target.result}" alt="Selected Image" width="20%" height="20%"/>`;
								        };
									...
									function uploadImage(imageNumber) {
										...
										var fileName = $("#" + fileInputId)
												.val().split('/').pop()
												.split('\\').pop();
										$(".upload-name[name='sel_img']")
												.eq(imageNumber - 1).val(
														fileName); // 각 input 요소에 대해 파일 이름 설정
									...
									<div class="form-group">
										...
										<label for="spec">제품 주요 사양	</label> <input
											type="text" class="form-control" id="spec"
											name="spec">
												<br>
										<label for="dt_use">사용기한 또는 개봉 후 사용기간		</label> <input
											type="text" class="form-control" id="dt_use"
											name="dt_use">
												<br>
									...
									<button type="button" class="btn btn-danger mr-2" onclick="javascript:writeProc();">판매등록</button>
